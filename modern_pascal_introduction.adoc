...
if A <> 0 then
  if B <> 0 then
    AIsNonzeroAndBToo else
    AIsNonzeroButBIsZero;
----

Wrapping the nested `if` inside `begin` ... `end` is often better than the previous example. It's more obvious to the reader, even if you mess up the indentation. And when you add / remove some `else` clause in the code below, it's obvious to which condition it applies (the `A` test or the `B` test), so it's less error-prone.
...
The _relational (comparison)_ operators are `=`, `<>`, `>`, `<`, `\<=`, `>=`. If you're accustomed to C-like languages, note that in Pascal you compare two values (check are they equal) using a simple equality `A = B` (unlike in C where you use `A == B`). The special _assignment_ operator in Pascal is `:=`.
...
*In the Castle Game Engine:* use `WritelnLog`, never `Writeln`. This will be always directed to some useful output. On Unix, standard output. On Windows GUI application, log file. On Android, the _Android logging facility_ (visible when you use `adb logcat`).
...
It is not allowed to have _circular unit dependencies in the interface_. That is, two units cannot use each other in the interface section. The reason is that everything must be declared before it's used. In order to "understand" (e.g. decide on a memory layout of a structures) the interface section of a unit, the compiler must first "understand" all the units it uses in the interface section. Pascal language follows this rule strictly, which results in fast compilation, but also makes circular dependencies _between units interfaces_ impossible.
...
// INCORRECT example.
// This is what compiles "sees" when trying to compile previous example
...
I advice using `FreeAndNil(A)` always, without exceptions, and never to call directly the `Free` method or `Destroy` destructor. The _Castle Game Engine_ does it like that. It provides a nice assertion that _all references are either nil, or point to valid instances_.
...
For dynamic-length lists of stuff, I advice using generic classes from the `FGL` unit. Use `TFPGList` for lists of primitives (or records or old-style objects), `TFPGObjectList` for a list of class instances. *In the Castle Game Engine:* You can also use `CastleGenericLists` with `TGenericStructList` for a list of records or old-style objects, this workarounds the problem of impossibility to override their operators in earlier FPC versions.
...
Records are useful when you need speed:

* Records do not have any constructor or destructor. You just define a variable of a record type. It has undefined contents (memory garbage) at the beginning (except for auto-managed types, like strings, they are guaranteed to be initialized to be empty).
* Arrays of records are nicely linear in memory, so they are cache-friendly.
* The memory layout of records (size, padding between fields) is clearly defined in some situations: you declare the _C layout_, or when you use `packed record`. This is nice if you need to pass records around to libraries written in other programming languages, or when you use records to make dirty low-level tricks (like unsafe typecasting one type to another, being aware of their memory representation).
* Records can also have `case` parts, which work like _unions_ in C-like languages. They allows to treat the same memory piece as a different type, depending on your needs. As such, this allows for greater memory efficiency in some cases, or for more _dirty, low-level unsafe tricks_:)
...
### Typecasting interfaces without "is" operator
...
It works for all `C1`, `C2`, `C3` instances in the examples in previous sections. If executed, it would make a run-time error in case of `C3`, that does not implement `IMyInterface` (we secure from it by checking with `Cx is IMyInterface` previously).
...
In this case, the typecast must be valid at compile-time. So it will compile OK for `C1` and `C2` (that are declared as classes that implement `IMyInterface`). But it will not compile for `C3`.

So this typecast works just like with classes. If an instance of class `TMyClass` is required, you can always use there a variable that is declared with a class of `TMyClass`, or `TMyClass` *descendant*.
