...
You can also _force_ the compiler to start searching from an ancestor class by using:
...
TIP: Go ahead, change the implementation of `TMyClass2.MyOtherMethod` above to use `inherited MyMethod`, and see what happens.
...
Since using `inherited` to call a method with the same name, with the same arguments, is a very often case, there is a special shortcut for it: you can just write `inherited;` (`inherited` keyword followed immediately by a semicolon). This means "_call an inherited method with the same name, passing it the same arguments as the current method_".

TIP: In the above example, all the `inherited Xxx;` calls could be replaced by a simple `inherited;`.

Note that you usually want to make the `MyMethod` _virtual_ in such case. More about the virtual methods in section below. But the `inherited` keyword works regardless if the method is virtual or not. It merely means that the compiler starts searching the method in an ancestor.
...
Internally, virtual methods work by having so-called _virtual method table_ associated with each class. This table is a list of pointers to the implementations of virtual methods for this class. When calling the `Eat` method, the compiler looks into a virtual method table associated with the actual class of `Fruit`, and uses a pointer to the `Eat` method defined there.
...
The `Destroy` destructor in the `TObject` is defined as a _virtual_ method, so you should always mark it with the `override` keyword in the descendants. This makes the `Free` method work correcrtly. Recall how the virtual methods work from the <<virtual-methods-section>>.
