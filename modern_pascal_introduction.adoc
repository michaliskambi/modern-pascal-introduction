...
An "exception" must a class instance.

- The compiler does not enforce much. You just must call `raise XXX` where `XXX` is an instance of any class. Any class (so, anything descending from `TObject`) is fine.
...
...
  ...
  on E: ESomeOtherException do
    ...
    Writeln('Unexpected exception occurred');
// WARNING: DO NOT FOLLOW THIS EXAMPLE WITHOUT READING A WARNING BELOW ABOUT "CAPTURING ALL EXCEPTIONS"
end;
----

You can also react to any exception raised:
...
// WARNING: DO NOT v THIS EXAMPLE WITHOUT READING A WARNING BELOW ABOUT "CAPTURING ALL EXCEPTIONS"
----

_Usually you should only catch exceptions of the specific class, that are appropriate in your case (you know what to do in case of this particular problem). Be careful with catching exceptions of a general type (like catching any `Exception` or any `TObject`), as you may easily catch too much, and later cause troubles when debugging other problems._ As in all programming languages with exceptions, the good rule to follow is to _never capture an exception that you do not know how to handle_. In particular, do not capture an exception just as a simple workaround of the problem, without investigating first _why_ the exception occurs. Is it a problem in user input (then you can report it)? Or is it a bug in your code logic (then you should fix the code, to avoid the exception)? And confirm that "catching" the exception at given place results in a correct code in all scenarios.

Another way to capture all exception is to use:
...
// WARNING: DO NOT FOLLOW THIS EXAMPLE WITHOUT READING A WARNING ABOVE ABOUT "CAPTURING ALL EXCEPTIONS"
----
...
...
  ...
  begin
    if E.InvalidUrl = 'http://example.com/blablah.wav' then
      Writeln('Warning: loading http://example.com/blablah.wav failed, this is a known problem that we can ignore now')
    else
      raise;
  end;
end;
...
### Finally (doing things regardless if exception occurred)

Often you use `try .. finally .. end` construction to free an instance of object, regardless if an exception occurred during some operation on this object. The way to write it looks like this:
...
This works reliably always, and does not cause memory leaks, even if `MyInstance.DoSomething` or `MyInstance.DoSomethingElse` raises an exception.
...
Note: you could also make a valid argument that it could be split into 3 separate procedures, one calling each other, in this simple case.
