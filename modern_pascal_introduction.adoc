...
Without the GUIDs, the `is` operator returns `true` always if your class supports _any_ interface. And the `Supports(ObjectInstance, IMyInterface)` magic function refuses to be compiled for interfaces without GUID (which is actually a good thing, it's better to refuse to compile than to blindly return `true` and mask the problem). This is true for both CORBA and COM interfaces, as of FPC 3.0.0.
