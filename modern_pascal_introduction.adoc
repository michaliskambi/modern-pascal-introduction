...
This is quite powerful feature that often allows you to split a long routine (_function, procedure, method_) into many smaller ones _easily_. You simply define a helper routine within another one.

The local routine can freely access (read, write) all the parameters of a parent routine, _and all the local variables of the parent that were declared above it_. This is very powerful, makes using local routines really easy, to split those long routines without much effort (as you don't have to pass around all the necessary information in parameters). It may also get somewhat unreadable, if you allow too many local functions to use the same local variable. So use this feature carefully.
