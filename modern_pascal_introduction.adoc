...
The first big difference between `try..except` and `try..finally` is that `try..except` "catches" the exception allowing you to handle the situation while `try..finally` will re-raise the exception after it has finished the execution. Another big difference is that `try..finally` executes in most possible scenarios of "leaving" the code block, not only through exceptions. E.g. consider the following code:
