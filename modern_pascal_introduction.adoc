...
----
+
If `Cx` is declared as a class (like `TMyClass2`), then this is a typecast that must be valid at compile-time. Note that this syntax is inconsistent with classes typecasts. In case of classes, writing `TMyClass(Cx)` is an _unsafe, unchecked_ typecast. In case of interfaces, writing `IMyInterface(Cx)` is a safe, fast (checked at compile-time) typecast.
+
However, if `Cx` is declared as an interface (like `IMyInterface2`) then this is (consistenlty with typecasting on classes) an _unsafe_ typecast. This typecast is not checked (not even at runtime), and bad things will happen if you cast to an incorrect interface.
