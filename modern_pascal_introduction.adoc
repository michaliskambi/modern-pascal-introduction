...
Wrapping the nested `if` inside `begin` ... `end` is often better than the previous example. It's more obvious to the reader, even if you mess up the identation. And when you add / remove some `else` clause in the code below, it's obvious to which condition it applies (the `A` test or the `B` test), so it's less error-prone.
