...
[source,pascal]
----
include::functions_primitives.lpr[]
----
...
+
Note that it's OK for _getter_ to have some _invisible_ side-effect, for example to cache a value of some calculation (known to produce the same results for given instance), to return it faster next time. This is in fact one of the cool possilities of a "getter" function.
...
The explanation of `private` and `protected` visibility above is not precisely true. The code _in the same unit_ can overcome their limits, and access the `private` and `protected` stuff freely. Sometimes this is a nice feature, allows you to implement tighly-connected classes. Use `strict private` or `strict protected` to secure your classes more tightly. See <<private and strict private>> section.
...
begin
  ...
  { With O2, things are more difficult. It's not nil, but it's invalid.
    ...
    behaviour - maybe an access violation exception, maybe a garbage data
    returned. }
end;
----
...
* The other solution is to use `TComponent` class "free notification" mechanism. One component get be notified when another component is freed, and thus set it's reference to `nil`.
+
...
----
+
Sometimes this is better than writing a long code with many `{$ifdef UNIX}`, `{$ifdef MSWINDOWS}` intermixed with normal code (variable declarations, routine implementation). The code is more readable this way. You can even use this technique more aggressively, by using the `-Fi` command-line option of FPC to include some subdirectiries only for specific platforms. Then you can have many version of include file `{$I my platform_specific_implementation.inc}` and you simply include them, letting the compiler find the correct version.
