...
The language and run-time library offer various flexible containers. There are a number of non-generic classes (like `TList` and `TObjectList` from the `Contnrs` unit), there are also dynamic arrays (`array of TMyType`). But to get the most flexibility *and* type-safety, I advice using *generic containers* for most of your needs. See the <<Generics>> for an explanation of this language feature.
...
In `FGL` unit, the `TFPGGList` and `TFPGMap` can be only used for types for which the equality operator (=) is defined. For `TFPGMap`, also the "greater than" (<) operator must be defined. If you want to use them with records, you have to overload their operators as shown in the <<Operator overloading>>.
