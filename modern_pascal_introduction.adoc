...
If you copy a reference to the instance, such that you have two references to the same memory, and then one of them is freed -- the other one becomes a _"dangling pointer"_. It should not be accessed, as it points to a memory that is no longer allocated. Accessing it may result in a runtime error (most often a SIGSEGV), or garbage being returned (as the memory may be reused for other stuff in your program).
...
1. At the end of this block, the `Obj1` is `nil`. If some code has to access it, it can reliably use `if Obj1 <> nil ...` to avoid calling methods on a freed instance, like
+
