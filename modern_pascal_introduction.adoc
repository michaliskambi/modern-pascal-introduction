...
Note: the default interface kind (if we would omit the `{$interfaces corba}` declaration) is COM. The _COM_ types of interfaces are unfortunately an _embarrassment to the language_ (details in the next section). That's why I would advice to just use `{$interfaces corba}` in all modern code for interfaces.
