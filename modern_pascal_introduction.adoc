...
_Published properties_ are the basis of a _serialization_ (also known as _streaming components_) in Pascal. Serialization term is derived from "Series" word, which means that some object data is represented as a linear data series, such as a memory or a file record.
