...
Има много книги и ресурси за Паскал, но в прекалено много от тях се говори само за стария Паскал, без класове, модули или генерици.
...
Затова написах бързо въведение в това, което наричам *модерен Обектен Паскал*. Повечето от програмистите, които го използват, всъщност не го наричат _"модерен Обектен Паскал"_, Просто го наричаме _"нашия Паскал"_. Но когато представям езика, чувствам, че е важно да подчертая, че това е модерен, обектно-ориентиран език. Той се е развил *много* от времето на стария(Turbo) Паскал, който много хора са учили преди време в училищата. Функционална, той е доста подобен на C++, Java или C#.

* Той има всички съвременни функции, които очаквате - класове, модули, интерфейси, генерици ...
* Той се компилира до бърз машинен код,
* Той е много типово безопасен,
* Той е език от високо ниво, но също така може да е и от ниско ако желаете.

Освен това има отличен, преносим компилатор с отворен код, наречен _Free Pascal Compiler_, http://freepascal.org/ . И придружаващо IDE (редактор, Debugger, библиотека от визуални компоненти, дизайнер на форми), наречена _Lazarus_ http://lazarus.freepascal.org/ . Самият аз съм създател на _Castle Game Engine_, https://castle-engine.io/ , която е 3D и 2D библиотека с отворен код, използваща модерен Паскал за създаване на игри на много платформи (Windows, Linux, macOS, Android, iOS, Nintendo Switch; също се подготвя и WebGL).

Това въведение е насочено най-вече към програмисти, които вече имат опит в програмирането на други езици. Тук няма да разглеждаме значенията на някои универсални концепции, като _ "Какво е клас" _, ще покажем само как да ги правим в Паскал.
...
Това е пълна програма, която можете да _компилирате_ и _стартирате_.
...
1. Нещо да изглегда като поле (да може да се чете и записва) но под него да има методи за четене (_getter_) и запис (_setter_). Често се използва за получаване на странични ефекти (напр. обновяване на екрана) всеки път когато стойността се промени;
