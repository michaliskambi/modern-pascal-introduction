...
Конструкции `if .. then` или `if .. then .. else` запускает некий код, когда некое условие удовлетворительно. В отличии от C-подобных языков, в Pascal тебе не нужно ставить в скобки условие.
...
Оператор `else` относится к полседнему условию `if`. Поэтому оно работает, как ты и ожидаешь:
